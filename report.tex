...
\begin{lemma}
The automorphism $\tau$ is unipotent of degree $n$. Moreover,
\begin{align*}
\rotation{E_i} &= E_{i+1}\\
\rotation{F_i} &= F_{i+1}\\
\rotation{1_\lambda} &= 1_{\lambda[-1]}
...
\begin{proof}
The automorphism $\rotation{}$ is unipotent of order $n$, by the $(n,n)$-periodicity condition on $\matrices$.
...
\rotation{(1_\lambda)} = 1_{\lambda[-1]}
\end{align*}
for each $\lambda\in\compositions$. In particular,
\begin{align*}
\rotation{(E_i)} &= E_{i-1}\\
\rotation{(F_i)} &= F_{i-1}
\end{align*}
...
Using the fundamental multiplication rules \ref{lemma:fundamental-multiplication-rule-generic} and \ref{lemma:multiplication-rule-generic-right}, for each $i\modn$,
\begin{align*}

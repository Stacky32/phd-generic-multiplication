...
and
\begin{equation*}
\antipode{\antipode{e_A}}=e_A
\end{equation*}
for all $A,B\in\matrices$. Moreover,
\begin{align*}
\antipode{E_i} &= F_i\\
\antipode{F_i} &= E_i\\
...
Let $S$ be the $\integers$-module automorphism of $\generic$ given by
\begin{equation*}
\antipode{e_A}=e_{A^\transpose}
\end{equation*}
for each $A\in\matrices$.
...
The map $S$ is an idempotent $\integers$-algebra antihomomorphism. In particular,
\begin{equation*}
\antipode{e_A\ast e_B} = \antipode{e_B}\ast \antipode{e_A}
\end{equation*}
for each $A,B\in\matrices$ and $S\circ S$ is the identity morphism on $\generic$.
\end{lemma}
...
\end{equation*}
so $S\circ S$ is the identity map on $\qschur$.

For each $i\in\modn$ and $\lambda\in\compositions$ with $\lambda_{i+1}>0$,
\begin{equation*}
\antipode{E_{i,\lambda}} = F_{i,\lambda+\alpha_i},
\end{equation*}
for each $i\in\modn$ and $\lambda\in\compositions$ with $\lambda_i>0$,
\begin{equation*}
\antipode{F_{i,\lambda}} = E_{i,\lambda - \alpha_i}
\end{equation*}
and
...
Finally, the equations involving $F_i$ and $F_{i+1}$ follow by applying the transpose involution, since
\begin{align*}
F_iF_{i+1}F_i^2 - F_i^2F_{i+1}F_i

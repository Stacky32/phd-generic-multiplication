...
\end{equation*}
called the affine $0$-Schur algebra.
...
\end{equation*}
and $\qint{0}=0$. For $i,j\in\integers$, let $\elem{i}{j}$ be the elementary periodic matrix with entries equal to $1$ in positions $(i+cn,j+cn)$ for $c\in\integers$ and all other entries equal to zero.
...
{\color{red}
quiver and relations with homomorphism; standard path for matrix; conjecture: image of a standard path.
}
...
In order to gain geometric insight on the product of $G$-orbits in $\dblflags$ we consider finite dimensional slices of the orbits and orbit products together with the action of a finite dimensional quotient of a stabiliser in $G$. Given $A\in\matrices$ the row sum of $A$ is the composition $\ro{A}$ given by adding up the entries in each row and the column sum $\co{A}$ is the composition given by adding up the entries in each column of $A$.

For $A,B\in\matrices$ and $L\in\flags_{\ro{A}}$ we consider the spaces
\begin{equation*}
...
\end{equation*}
together with the action of the stabiliser $G_L$.

\begin{lemma*}
The $G_L$-orbit $\xprod[L]{A}$ is an irreducible quasiprojective variety.
\end{lemma*}

\begin{lemma*}
The space $\yprod[L]{A,B}$ is an irreducible quasiprojective variety.
\end{lemma*}

\begin{lemma*}
The space $\xprod[L]{A,B}$ is irreducible and constructible.
\end{lemma*}
...
{\color{red}
factorising aperiodic basis elements; quiver and relations for generic algebra, with surjective map; conjecture: the presenting map is an isomorphism. relies on technical lemma; conjecture: isomorphism from generic algebra to 0-schur algebra. also depends on the technical lemma.}
...
\begin{equation*}
e_i^s e_{i+1}^s e_i - e_i^{s+1}e_{i+1}^s = 0
\end{equation*}
and
\begin{equation*}
e_i^s e_{i+1}^t - e_{i+1}^{t-s} e_i^s e_{i+1}^s = 0
\end{equation*}

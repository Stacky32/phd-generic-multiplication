...
Each element $g\in G_L$ defines an endomorphism $\overline{g}$ of $W$, given by
\begin{equation*}
\overline{g}(x+\epsilon^N L_0) = g(x) + \epsilon^N L_0,
\end{equation*}
for each $x\in\epsilon^{-1-N}L_0$. Given $g,h\in G_L$ and $x\in\epsilon^{-1-N}L_0$,
\begin{align*}
\overline{gh}(x+\epsilon^N L_0)
&= g(h(x))+\epsilon^N L_0\\
&= \overline{g}(h(x)+\epsilon^N L_0)\\
&= (\overline{g}\circ\overline{h})(x+\epsilon^N L_0),
\end{align*}
so $\overline{g}$ is an automorphism of $W$ with inverse $\overline{g^{-1}}$. This defines a group homomorphism
\begin{equation*}
\bar{\cdot}\colon G_L\to \GL{}{W}: g\mapsto\overline{g}
\end{equation*}
with kernel
\begin{equation*}
H_{N,L}\coloneqq \{g\in G_L:g(x)\in x+\epsilon^NL_0 \text{ for all } x\in\epsilon^{-1-N}L_0\},
\end{equation*}
so that $G_L/{H_{N,L}}$ is isomorphic to a subgroup of $\GL{}{W}$.
...
\begin{proof}
As a result of the first isomorphism theorem for groups, $G_L/{H_{N,L}}$ is isomorphic to the image of $G_L$ in $\GL{}{W}$, which will be described explicitly by homogeneous polynomial equations in the coordinate functions on $\GL{}{W}$, with respect to a fixed basis of $W$.

Let $\{\tilde{x}_1,\ldots,\tilde{x}_r\}$ be a basis of $L_n/{L_0}$ over $\complex$ adapted to the flag
\begin{equation*}
...
\end{equation*}
for each $i\in\{1,\ldots,n\}$. Choose $x_1,\ldots,x_r\in L_n$ with $\tilde{x}_i=x_i+L_0$ so that
\begin{equation*}
...
The parabolic subgroup $\mathcal{P}_\lambda$ in $\GL{r}{\complex}$ is a connected algebraic group and $M_r(\complex)$ is an irreducible affine variety, so the image of $G_L$ in $\GL{}{W}$ is a connected algebraic group and therefore $G_L/{H_{N,L}}$ is a connected algebraic group.
\end{proof}

\begin{lemma}\label{lemma:projective-varieties-of-cyclic-flags}
The set $\Pi_{N,\lambda}^a(L)$ is a projective algebraic variety for each $L\in\flags$, $N\in\naturals$, $\lambda\in\compositions$ and $a\in\naturals$ with $0\le a\le 2Nr$
\end{lemma}

\begin{proof}
Let $d_i = 2Nr -a +\lambda_1 +\cdots +\lambda_i$ for each $i=1,\ldots, n$. The correspondence between $\complex[\epsilon]$-submodules of $\epsilon^{-1-N}L_0$ which contain $\epsilon^N L_0$ and $\complex[\epsilon]$-submodules of $W$ defines an injective map
\begin{equation*}
...
Suppose $(U_1,\ldots,U_n)\in\mathcal{X}$. Then $U_i$ is a $\complex[\epsilon]$-module, since $\epsilon U_i\subset \epsilon U_n\subset U_1\subset U_i$, for each $i=1,\ldots,n$, so $U_i$ lifts uniquely to a $\complex[\epsilon]$-module $L_i'$ with $\epsilon^N L_0\subset L_i'\subset\epsilon^{-1-N}L_0$. Therefore $L_1',\ldots,L_n'$ are $\complex[\epsilon]$-lattices with $L_i\subset L_{i+1}$ for $i=1,\ldots,n-1$ and $\epsilon L_n'\subset L_1'$, with
\begin{equation*}
\dim\left(\epsilon^{-1-N}L_0/{L_n'}\right) = \dim\left(W/{U_n}\right) = (2N+1)r - d_n = a
\end{equation*}
and
\begin{equation*}
\dim\left(L_i'/{L_{i-1}'}\right) = \dim\left(U_i/{U_{i-1}}\right) = d_i - d_{i-1} = \lambda_i,
\end{equation*}
for each $i=2,\ldots,n$. Therefore there is a unique $L'\in\Pi_{N,\lambda}^a(L)$ such that $\rho(L')=(W_1,\ldots, W_n)$, where $L'$ is given by $L_{i+cn}'=\epsilon^{-c}L_i'$ for $i=1,\ldots,n$ and $c\in\integers$. It follows that $\IM{\rho}=\mathcal{X}$, which is a projective variety, so $\Pi_{N,\lambda}^a(L)$ is a projective variety.
\end{proof}
...
\end{equation*}
which shows $L'\in\Pi_{N,\lambda}^a(L)$. Therefore $\Pi_{N,\lambda}^a(L)$ is the closed subset of $\Pi_{N+1,\lambda}^{a+r}(L)$ defined by the conditions $\epsilon^N L_0\subset L_0'$ and $L_0'\subset\epsilon^{-N}L_0$, using Lemma \ref{lemma:grassmannian-incidence-lemmas}.
\end{proof}

...
\end{equation*}
where $1_\lambda$ is the basis element corresponding to the diagonal matrix with column vector equal to $\lambda$. There are distinguished elements $E_i$ and $F_i$ for $i\in\integers/{n \integers}$, where $E_i$ is the sum of those basis elements $e_A$ such that $a_{j,j+1}=1$ when $j=i$ modulo $n$ with all other off-diagonal entries are zero, and $F_i$ is the sum of those $e_A$ such that $a_{j+1,j}=1$ when $j=i$ modulo $n$ and all other off-diagonal entries are zero.
...
\end{align*}
for each $i\in\integers/{n\integers}$.
...
\end{align*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$, where $\alpha_i=\epsilon_i - \epsilon_{i+1}\in\integers^n$ is the simple root. 
...
f_if_j &-f_jf_i
\end{align*}
for $i,j\in\integers/{n \integers}$ such that $j\neq i\pm 1$;
\begin{align*}
e_ie_{i+1}^2 -&(1+q)e_{i+1}e_ie_{i+1} +qe_{i+1}^2e_i\\
e_i^2e_{i+1} -&(1+q)e_ie_{i+1}e_i +qe_{i+1}e_i^2\\
f_{i+1}^2f_i -&(1+q)f_{i+1}f_if_{i+1} +qf_if_{i+1}^2\\
f_{i+1}f_i^2 -&(1+q)f_if_{i+1}f_i +qf_i^2f_{i+1}
\end{align*}
for $i\in\integers/{n \integers}$;
\begin{equation*}
e_if_j-f_je_i
\end{equation*}
for $i,j\in\integers/{n \integers}$ such that $i\neq j$;
\begin{equation*}
e_if_i-f_ie_i - \sum_{\lambda\in\compositions}(\qint{\lambda_i}-\qint{\lambda_{i+1}})k_\lambda
\end{equation*}
for $i\in\integers/{n \integers}$, where
\begin{align*}
e_i &= \sum_{\lambda\in\compositions:\lambda_{i+1}>0} e_{i,\lambda}\\
f_i &= \sum_{\lambda\in\compositions:\lambda_i>0} f_{i,\lambda}
\end{align*}
...
\end{align*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$. We conjecture that if $\qfrac$ is a subring of $\rationals(q)$ such that the $q$-integers are invertible and $q$ is not invertible, then the induced $\qfrac$-algebra homomorphism $\qfrac\otimes\phi$ is an isomorphism. Surjectivity is proven in Proposition \ref{proposition:q-schur-presentation}.
...
\end{align*}
for $i,j\in\integers/{n \integers}$ and $\lambda\in\compositions$.

An element $A\in\matrices$ is said to be aperiodic if for every $s\neq 0$ there is $i\in\integers/{n \integers}$ such that $a_{i,i+s}=0$.
...
\end{align*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$. The image of $\rho$ is spanned by the aperiodic basis elements. If $r<n$ then $\rho$ is surjective.
\end{proposition*}
...
\Psi(F_i) &= F_i.
\end{align*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$.
\end{conjecture*}
...
A composition $\lambda\in\compositions$ is \emph{sincere} if $\lambda_i>0$ for each $i\in\integers/{n \integers}$ and otherwise $\lambda$ is \emph{insincere}.

For each $i\in\integers/{n \integers}$, let
\begin{equation*}
...
\end{equation*}
for each $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ with $\lambda_{i+1}>0$, and
\begin{equation*}
\antipode{(F_{i,\lambda})} = E_{i,\lambda -\alpha_i}
\end{equation*}
for each $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ with $\lambda_i>0$.
...
These fundamental multiplication rules for products with $E_i$ and $F_i$ in $\qschur$ are derived from the $v$-form of these expressions, where $q=v^2$, given by Lusztig in Proposition 3.5 \cite{lusztig99}.

\begin{lemma}\label{lemma:fundamental-multiplication-rule}
Given $A\in\matrices$ and $i\in\integers/{n \integers}$ with $\ro{A}_{i+1}>0$,
\begin{equation*}
E_ie_A = \sum_{p\in\integers:a_{i+1,p}>0} q^{\sum_{j>p}a_{i,j}} \qint{a_{i,p}+1} e_{A+\elem{i}{p} -\elem{i+1}{p}}.
\end{equation*}
Given $A\in\matrices$ and $i\in\integers/{n \integers}$ with $\ro{A}_i>0$,
\begin{equation*}
F_ie_A = \sum_{p\in\integers:a_{i,p}>0} q^{\sum_{j<p}a_{i+1,j}} \qint{a_{i+1,p}+1} e_{A+\elem{i+1}{p} -\elem{i}{p}}.
\end{equation*}
\end{lemma}

These formulas are still valid in the cases $E_ie_A=0$ and $F_ie_A=0$. If the convention that $e_B = 0$ whenever $B$ is not in $\matrices$ is used, then the conditions on $p$ in the above sums may be ignored.

\begin{corollary}\label{corollary:fundamental-right-multiplication-rules}
Given $A\in\matrices$ and $j\in\integers/{n \integers}$ with $\co{A}_{j+1}>0$,
\begin{equation*}
e_AF_j = \sum_{p\in\integers: a_{p,j+1}>0} q^{\sum_{i>p}a_{i,j}} \qint{a_{p,j}+1} e_{A+\elem{p}{j}-\elem{p}{j+1}}.
\end{equation*}
Given $A\in\matrices$ and $j\in\integers/{n \integers}$ with $\co{A}_{j}>0$,
\begin{equation*}
...
The following equations hold for each $A\in\matrices$ and $i,j\in\integers$:
\begin{align*}
...
Let $A\in\matrices$ and $i\in\integers/{n \integers}$ with $\ro{A}_{i+1}>0$. Using the fundamental multiplication rules \ref{lemma:fundamental-multiplication-rule} and Lemma \ref{lemma:numerical-degeneration},
\begin{equation*}
...
\end{equation*}
and, for each $i\in\integers/{n \integers}$,
\begin{equation*}
...
If $i,j\in\integers/{n \integers}$ and $i\neq j$, then
\begin{equation*}
E_iF_j - F_jE_i = 0.
\end{equation*}
For each $i\in\integers/{n \integers}$,
...
\begin{proof}
Denote $e_A$ by $\left[A\right]$. Fix $i,j\in\integers/{n \integers}$ with $i\neq j$. Then
\begin{align*}
...
An explicit version of these relations will be given after defining some terminology. Given $\lambda\in\compositions$ and $i\in\integers/{n \integers}$, say that $\lambda$ is internal with respect to $i$ if $\lambda-\alpha_i,\lambda+\alpha_i\in\compositions$. Say that $\lambda$ is initial with respect to $i$ if $\lambda-\alpha_i\notin\compositions$ and that $\lambda$ is final with respect to $i$ if $\lambda+\alpha_i\notin\compositions$. The expression for the commutator $\left[E_i,F_i\right]$ in Lemma \ref{lemma:q-commutators-E-F} gives the following relations in $\qschur$: if $\lambda$ is internal with respect to $i$ then
\begin{equation*}
...
For each $i\in\integers/{n \integers}$, define
\begin{equation*}
e_i = \sum_{\lambda\in\compositions:\lambda_{i+1}>0} e_{i,\lambda}
\end{equation*}
and
...
\phi(f_{i,\lambda} +\qrelations) &= F_{i,\lambda},
\end{align*}
for each $i\in\integers/{n \integers}$ and $\lambda\in\compositions$.
\end{proof}
...
\end{equation*}
for $j\in\integers/{n \integers}$. Call $p^+$ the \emph{positive part} of $p$ and $p^-$ the \emph{negative part} of $p$.
\end{definition}
...
\end{equation*}
for $i\in\integers/{n \integers}$ and $s\geq 1$, respectively.
\end{definition}
...
For $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ such that $\lambda_{i+1}>0$, define
\begin{equation*}
...
For $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ such that $\lambda_i>0$, define
\begin{equation*}
...
Let $i\in\integers/{n \integers}$ and $A\in\matrices$ and write $\mu=\ro{A}$.
...
Suppose $B\in\matrices$ with $B=A+\elem{i}{j}-\elem{i+1}{j}$ for some $j\in\integers$. For $s\in$ and $t\in\integers$,
\begin{equation*}
...
Suppose $B\in\matrices$ with $B=A+\elem{i+1}{j}-\elem{i}{j}$, for some $j\in\integers$. Then for $s\in\{1,\ldots,n\}$ and $t\in\integers$,
\begin{equation*}
d_{s,t}{(B)} = \begin{cases}
d_{s,t}{(A)} - 1 &: s=i \text{ and } t<j,\\
d_{s,t}{(A)} &: \text{ otherwise,}
...
For each $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ with $\lambda_{i+1}>0$,
\begin{equation*}
\antipode{(E_{i,\lambda})} = F_{i,\lambda+\alpha_i},
\end{equation*}
for each $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ with $\lambda_i>0$,
...
Let $i\in\integers/{n \integers}$ and $A\in\matrices$ and write $\lambda=\co{A}$.
...
\end{equation*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ with $\lambda_i>0$, and
\begin{equation*}
...
Let $A\in\matrices$ and $\mu=\ro{A}$. Suppose $i\in\integers/{n \integers}$ is such that $\mu_i>0$ and let
\begin{equation*}
...
The subalgebra of $\generic$ generated by $E_i$ and $F_i$ for $i\in\integers/{n \integers}$ and $1_\lambda$ for $\lambda\in\compositions$ has $\integers$-basis
\begin{equation*}
...
\begin{proof}
By definition, this subalgebra is spanned by the nonzero products in $E_i$ and $F_i$ for $i\in\integers/{n \integers}$ and $1_\lambda$ for $\lambda\in\compositions$, which are exactly the aperiodic basis elements, by Lemma \ref{lemma:words-are-aperiodic} and Lemma \ref{lemma:factorising-aperiodic-elements}.
\end{proof}

\begin{lemma}
In the case $r<n$, $\generic$ is generated by $E_i$ and $F_i$ for $i\in\integers/{n \integers}$ and $1_\lambda$ for $\lambda\in\compositions$.
\end{lemma}
...
\begin{equation*}
e_{i,\lambda}\colon\lambda\to\lambda+\alpha_i \text{ for } (i,\lambda)\in\integers/{n \integers}\times\compositions \text{ with } \lambda_{i+1}>0,
\end{equation*}
and
\begin{equation*}
f_{i,\lambda}\colon\lambda\to\lambda-\alpha_i \text{ for } (i,\lambda)\in\integers/{n \integers}\times\compositions \text{ with } \lambda_i>0.
\end{equation*}
...
f_if_j &- f_jf_i
\end{align*}
for $i,j\in\integers/{n \integers}$ with $j\neq i\pm 1$;
\begin{align*}
...
e_if_j -f_je_i
\end{equation*}
for $i,j\in\integers/{n \integers}$ with $i\neq j$;
\begin{equation*}
...
\end{equation*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ with $\lambda_i>0$ and $\lambda_{i+1}>0$;
\begin{equation*}
e_{i,\lambda-\alpha_i}f_{i,\lambda} - k_\lambda
\end{equation*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$ such that $\lambda_i>0$ and $\lambda_{i+1}=0$;
\begin{equation*}
...
\begin{proof}
Suppose $i,j\in\integers/{n \integers}$ with $j>i+1$, so $\{i,i+1\}$ and $\{j,j+1\}$ are disjoint, then
\begin{align*}
...
Using the fundamental multiplication rules \ref{lemma:fundamental-multiplication-rule-generic} and \ref{lemma:fundamental-multiplication-rule-generic-R}, for each $i\integers/{n \integers}$,
\begin{align*}
...
Suppose $i,j\in\integers/{n \integers}$ with $i\neq j$. Then
\begin{align*}
...
Finally, for $i\in\integers/{n \integers}$,
\begin{equation*}
...
\end{align*}
for all $i\in\integers/{n \integers}$ and $\lambda\in\compositions$. Thus $\generic$ may also be regarded as an algebra over $\integers\Gamma$ where the action of a path $p$ is given by
\begin{equation*}
...
\begin{proof}
The image of $\rho$ is the subalgebra of $\generic$ generated by $E_i$ and $F_i$ for $i\in\integers/{n \integers}$ and $1_\lambda$ for $\lambda\in\compositions$, which has $\integers$-basis
\begin{equation*}
...
\begin{proof}
If $p$ is a standard positive or negative path then $p$ is reduced, since the relations only involving the edges $e_i:i\in\integers/{n \integers}$ or $f_i:i\in\integers/{n \integers}$ are homogeneous polynomials, so any equivalent path is of the same length.
...
\end{align}
in $\integers\Gamma/{\zerorelations}$, for each $i\in\integers/{n \integers}$.
\end{lemma}
...
Let $p$ be a standard path in $\Gamma$. If $q$ is a path in $\Gamma$ with $q=pe_i$ or $q=pf_i$ for some $i\in\integers/{n \integers}$, then $q$ is congruent to a standard path modulo $\zerorelations$.
\end{conjecture}
...
\begin{proof}[Idea of proof.]
Let $p$ be a path in $\Gamma$ and proceed by induction on the length of $p$. If $p$ has length zero then $p=k_\mu$ for some $\mu\in\compositions$, so $p$ is a standard path. If $p$ has length one then $p=k_\mu e_i$ or $p=k_\mu f_i$ for some $\mu\in\compositions$ and $i\in\integers/{n \integers}$.
...
\phi(k_\lambda +\qrelations)&= 1_\lambda,
\end{align*}
for $i\in\integers/{n \integers}$ and $\lambda\in\compositions$. Let $\phi_0$ be the $\integers$-algebra homomorphism
\begin{equation*}
...
\end{equation*}
for $i\in\integers/{n \integers}$ and there are likely to be more general relations with arbitrary exponents for $e_{i-1},\ldots,e_{i+1}$ and $f_{i+1},\ldots,f_{i-1}$ respectively.

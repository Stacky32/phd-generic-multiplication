...
\begin{proof}
Let $A\in\matrices$ and proceed by induction on the weight of $A$. If $\weight{A}=0$ or $\weight{A}=1$ then $e_A$ is in the image of $\phi_\qfrac$ since
\begin{align*}
\phi_\qfrac(k_\lambda +\qfrac\qrelations) &= 1_\lambda\\
\phi_\qfrac(e_{i,\lambda} +\qfrac\qrelations) &= E_{i,\lambda}\\
\phi_\qfrac(f_{i,\lambda} +\qfrac\qrelations) &= F_{i,\lambda}.
...
Assume $\weight{A}>1$ and the image of $\phi_\qfrac$ contains those basis elements of lower weight. First suppose $A$ has nonzero entries above the diagonal and write $h=\max\{j-i:a_{i,j}>0\}$. Fix $j$ such that $a_{j-h,h}=0$ and $a_{j+1-h,j+1}>0$ and let $B=A+\elem{j+1-h}{j}-\elem{j+1-h}{j+1}$. Then
\begin{equation*}
e_BF_j = \qint{b_{j+1-h,j+1} +1}e_A +\sum_{t>j+1-h:b_{t,j}>0} q^{\sum_{s<t} b_{s,j+1}} \qint{b_{s,j+1}+1} e_{B+\elem{s}{j+1}-\elem{s}{j}},
\end{equation*}
using Corollary \ref{corollary:fundamental-right-multiplication-rules}. Each term in the sum on the right hand side is of strictly lower weight than $A$ so is in the image of $\phi_\qfrac$ by induction. The image of $\phi_\qfrac$ contains $e_B$ since $\weight{B}=\weight{A}-1$, so $e_BF_j$ is also in the image of $\phi_\qfrac$. The coefficient of $e_A$ is invertible in $\qfrac$ so it follows that $e_A$ is in the image of $\phi_\qfrac$.

If there are nonzero entries in $A$ below the diagonal then $e_{A^\transpose}$ is in the image of $\phi_\qfrac$, which is closed under the transpose operation, so $e_A$ is in the image of $\phi_\qfrac$.
\end{proof}
